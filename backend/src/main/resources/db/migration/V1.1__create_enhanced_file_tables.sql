...
CREATE TABLE IF NOT EXISTS sys_file_reference (
    ...
    CONSTRAINT fk_file_reference_file FOREIGN KEY (file_id) 
        REFERENCES sys_file_enhanced (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='文件引用关系表';

-- 迁移现有文件数据到新表（如果有旧表）
-- INSERT INTO sys_file_enhanced (id, stored_name, access_url, upload_time, file_category)
-- SELECT id, file_name, url, upload_time, 'default'
-- FROM sys_file
-- WHERE NOT EXISTS (
--     SELECT 1 FROM sys_file_enhanced WHERE sys_file_enhanced.id = sys_file.id
-- );